...
- Keep NiPSysGrowFadeModifier (controls particle size — without it, base_scale=0 makes particles invisible). Set base_scale=1.0. base_scale patch makes the block 29 bytes = correct Skyrim size (NiPSysModifier parent 13 + own 16). Vanilla Skyrim uses BSPSysScaleModifier instead (0/250 vanilla particle meshes ship NiPSysGrowFadeModifier) but the engine still handles a valid one.
