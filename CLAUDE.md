...
- **NIF bhk conversion details** (Session 19+):
  ...
  - **This regressed once (fixed 2026-07)**: the wrap pass was changed to move bhkCollisionObject onto the inner rotated NiNode "so Havok reads the NiNode world transform" — WRONG. Caused intermittent (~50%) CTDs in `hkpCollisionDispatcher` when the character proxy touched the shape (castleint2way.nif in AnvilCastleGreatHall; crash stack shows hkpAllCdPointTempCollector + bhkCharProxyController). `_convert_collision` must also pass `actual_root` (not the owning node) to `_convert_shape` so bhkCompressedMeshShape.target = root. Regression test: `TestCollisionTargetPointsToRoot::test_static_collision_stays_on_root_when_wrapped`.
