...
- **NIF bhk conversion details** (Session 19+):
  ...
  - **MOPP_RL emits CHUNKED MOPP bytecode (root-caused 2026-07)**: MOPP_RL builds with Havok chunk subdivision enabled (SPU/PS3 streaming feature) — the code contains 5-byte `0x70 <abs32>` chunk-jump opcodes targeting 16-byte-aligned sub-trees appended after the main tree, with 0xCD alignment filler between them (the "0xCD garbage" in mopp_data is padding, NOT the whole story). Vanilla Skyrim never ships chunked MOPPs (0 of 400 vanilla meshes) and the PC engine mis-executes 0x70 → runaway MOPP VM → intermittent EXCEPTION_STACK_OVERFLOW in `hkpCollisionDispatcher` when the character proxy/NiPick query descends a 0x70 branch (the "~50% of the time walking over castleint2way.nif" crash; stackhallentrance01 had a 0x70 as the first branch child = ~half of all queries). Fix: `asset_convert/mopp.py::dechunk_mopp()` — delimits every region (main + chunks), lays them out in topological order of the jump graph (chunk jumps can point BACKWARD), copies region bytes verbatim (internal relative jumps stay valid), patches each 0x70 in place to a forward `0x06 JUMP16` (vanilla-proven opcode) + 2 dead bytes, zeroes unreachable bytes, and verifies (clean walk, no 0x70, identical shape-key set) before/after. Called from `_extract_mopp_result()`; on failure the caller falls back to packed-shape-without-MOPP.
  - `asset_convert/mopp.py::walk_mopp()` is a full MOPP VM symbolic walker (PyFFI's parse_mopp opcode table + Skyrim-era opcodes: 0x52 TERM24, 0x29-0x2B DOUBLE_CUT24, 0x70 CHUNK_JUMP32), validated clean against 400 vanilla meshes. CLI: `python tools/mopp_validator.py <nif_or_dir> [--verbose|--summary|--histogram]`. Vanilla opcode set observed: 0x01-0x06, 0x09-0x0B, 0x10-0x1C, 0x20-0x28 (0x29-0x2B rare), 0x30-0x53 — never 0x07/0x08/0x70; emit only these.
  ...
  - **Root rotation wrap + collision (settled 2026-07 after bugs in BOTH directions)**: when the wrap pass zeroes the root transform L=(R,T) and moves it to an inner NiNode, the collision object STAYS on the root BSFadeNode **and L is composed into the rigid body**: bodyT' = L ∘ bodyT (`bake_node_transform_into_body()` in collision.py). The engine places a root collision body at REFR ∘ bodyT — node transforms in the chain DO apply (NifSkope glnode.cpp: `worldTrans() * bodyT`), so:
    - Collision moved to the inner rotated NiNode → placement correct, but intermittent (~50%) CTD in `hkpCollisionDispatcher` when the character proxy touches the shape (castleint2way.nif in AnvilCastleGreatHall; stack shows hkpAllCdPointTempCollector + bhkCharProxyController). Static collision on a child NiNode is ALWAYS wrong.
    - Collision left on root WITHOUT composing L (the old "Oblivion bhk values are already world-space" claim was WRONG — source body quats are identity; shape data is root-local) → no crash but collision rotated relative to the mesh (stackhallentrance01: 90° off; castleint2way's 180° case half-worked because the hall is near-symmetric).
    - `_convert_collision` must pass `actual_root` (not the owning node) to `_convert_shape` so bhkCompressedMeshShape.target = root.
    - Non-T bhkRigidBody under a rotated root is promoted to bhkRigidBodyT (empty PyFFI subclass — class swap) so it can hold L.
    - Conventions: PyFFI `m_ij` names are the TRANSPOSE of the engine's column-vector matrix (file order m11,m21,m31,… fills engine rows); rigid body rotation is QuaternionXYZW; body translation stays in Oblivion Havok units (game/7) until `_convert_collision` scales it by 0.1. Regression test: `TestCollisionTargetPointsToRoot::test_static_collision_stays_on_root_when_wrapped` (the 90° case catches conjugate/transpose convention errors).
