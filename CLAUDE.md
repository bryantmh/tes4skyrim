...
- **Collision sanity checker**: `python tools/collision_sanity.py <nif_or_dir_or_listfile.txt> [--constraints] [--quiet]` — Walks all bhk blocks: NaN/Inf sweep, degenerate hulls/lists, non-unit constraint axes, hinge limit ordering; `--constraints` dumps full descriptor values for source-vs-converted comparison.
