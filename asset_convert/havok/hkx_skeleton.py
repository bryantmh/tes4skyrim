...
asset_convert/havok/hkx_ragdoll.py and merged into the same packfile.
...
apply_patches()
from asset_convert.havok.hkx_xml import (HkxPackfile, compile_hkx, decompile_hkx,
                                   fmt_vec)
...
def mat33_to_quat_xyzw(m) -> tuple:
...
def quat_xyzw_to_mat33(q) -> list:
    """Inverse of mat33_to_quat_xyzw (row-convention 3x3), for validation."""
    x, y, z, w = q
    return [
...
def collect_bones(root_node) -> list:
    ...
    def visit(node, parent_idx):
        ...
        bones.append(Bone(
            ...
            quat_xyzw=mat33_to_quat_xyzw(node.rotation),
...
def build_skeleton_xml(bones: list, skeleton_nif_path: str = None) -> str:
    ...
    if parts:                       # extracted above, before the anim skeleton
        from asset_convert.havok.hkx_ragdoll import emit_ragdoll
