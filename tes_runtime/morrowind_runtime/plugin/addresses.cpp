...
bool VersionDb::Load(std::uint32_t runtimeVersion) {
    ...
    // 1.5.97 all 62 Native<> ids are PRESENT and every one names a different
    // function. A hit there is a wrong address, not a missing one, so the
    // SE generation needs its own ids (ids_se.h) rather than these.
