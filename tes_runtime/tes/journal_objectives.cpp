...
//: Milliseconds between polls: 30 a second, the rate it had on the Morrowind object tick.
constexpr int kTickMs = 33;

//: The co-save record: which journal text each quest objective was shown with.
constexpr UInt32 kRecordJournal = 'JRNL';
...
// 🛑 Whether a GAME is loaded: the player standing in SOME cell, never whether
// that cell has a name. The main menu still runs the task pump, and a poll
// there would prime on an objective array that is about to be replaced.
// See: docs/commentary/morrowind_runtime.md#the-tick-is-gated-on-a-loaded-game
bool PlayerInWorld() {
    void* player = g_playerSlot ? *reinterpret_cast<void**>(g_playerSlot) : nullptr;
    return player && g_parentCell && g_api.vm && g_parentCell(g_api.vm, 0, player);
}

void Tick() {
    if (PlayerInWorld()) PollJournal();
}
...
bool InstallJournal() {
    ...
    g_parentCell = reinterpret_cast<ParentCellFn>(
        Resolve("ObjectReference.GetParentCell", ids::kRefGetParentCell, nullptr));
    ...
    if (!g_playerSlot || !g_lookupForm || !g_parentCell || !g_instanceText) {
...
void StartJournalTick() {
    const bool started = StartMainThreadTick(kTickMs, Tick);
    Log("journal: poll %s every %d ms", started ? "started" : "NOT started -- no task interface",
        kTickMs);
}

void JournalSave(SKSESerializationInterface* intfc) {
    const std::string body = g_journal.Serialize();
    const bool ok = intfc->OpenRecord(kRecordJournal, kJournalVersion) &&
                    intfc->WriteRecordData(body.data(), static_cast<UInt32>(body.size()));
    Log("journal: saved %zu record(s) -- %s", g_journal.size(), ok ? "ok" : "WRITE FAILED");
}
...
void JournalLoad(SKSESerializationInterface* intfc) {
    ...
    while (intfc->GetNextRecordInfo(&type, &version, &length)) {
        if (type != kRecordJournal || version > kJournalVersion) {
            Log("journal: skipped co-save record %08X v%u (%u bytes)", type, version, length);
            continue;
        }
        std::string body(length, '\0');
        body.resize(length ? intfc->ReadRecordData(body.data(), length) : 0);
        const std::size_t kept = g_journal.Deserialize(
            body, [intfc](std::uint32_t saved, std::uint32_t* now) {
                return intfc->ResolveFormId(saved, now);
            });
        Log("journal: loaded %zu record(s)", kept);
    }
}

void JournalRevert(SKSESerializationInterface*) {
    g_journal.Reset();
    ResetJournalPoll();
}
