...
void StartCrimeTick() {
    if (g_ticking || g_pools.empty() || g_jails.empty()) return;
    g_ticking = StartMainThreadTick(kTickMs, Update);
    Log("crime: jail tick %s every %d ms", g_ticking ? "started" : "NOT started", kTickMs);
}
