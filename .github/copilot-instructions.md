...
- **NIF bhk conversion details** (Session 19+):
  ...
  - bhkMoppBvTreeShape.build_type: MOPP_RL.exe writes 0xCD (uninit memory) for build_type. Must set build_type=1 (BUILT_WITHOUT_CHUNK_SUBDIVISION) AFTER collision conversion. IMPORTANT: `data.blocks` is a cached list from read-time — newly created blocks (from MOPP_RL) are NOT in it. Must walk `root.tree()` instead.
