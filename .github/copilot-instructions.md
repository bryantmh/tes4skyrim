...
- **NIF bhk conversion details** (Session 19+):
  ...
  - `bhkCompressedMeshShape.target` must point to the specific NiNode that owns the collision_object (use the node passed to `_convert_collision`, NOT `actual_root`). When root-rotation wrapping creates an inner NiNode that owns the collision, target = inner NiNode (which carries the rotation transform). Setting target to BSFadeNode (identity) would misplace collision at origin.
