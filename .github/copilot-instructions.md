...
- **Per-quest topic ownership**: Bark topics (GREETING, combat, detection, misc) → universal TES4Dialogue quest (SGE). Conversation topics WITH quest association → original TES4 quest (remapped FormID). Conversation topics WITHOUT quest → universal quest. This prevents non-SGE quest topics from appearing at game start (e.g., Azzan's "Rats" only appears when FGC01Rats starts). DLBR references the owning quest, not always the universal quest.
...
- **DIAL BNAM**: Present on ALL non-bark conversation topics (links to DLBR). Bark topics must NOT have BNAM. TCLT target topics (Type 1 chain) ALSO get BNAM — vanilla Skyrim requires BNAM on ALL CUST topics for the engine to route dialog to them.
...
- **Quest running gating (QSTI restoration)**: In Oblivion, QSTI quest association implicitly gates dialog — INFOs only show when their QSTI quest is running. Since we put ALL dialog on the universal TES4Dialogue quest (SGE, always running), we lost this gating. Fix: `build_dialog_groups()` builds `sge_quest_fids` set from QUST records (DATA.Flags & 0x01), then for each DIAL whose original `Quest[0]` is NOT SGE, injects `GetQuestRunning(remapped_quest_fid)==1.0` as the FIRST CTDA on every child INFO. **CRITICAL: Only applies to non-bark (conversation) topics.** Bark topics (HELO, GBYE, combat, detection, misc) must NOT get quest-running gating — their original TES4 conditions already provide proper filtering, and injecting GetQuestRunning on non-SGE quests blocks ALL greetings (only the fallback "Hello." would work). 208 SGE quests, 4193 INFOs gated.
