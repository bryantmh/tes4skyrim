...
|--------|---------------------------|-------|
| **DIAL** | EDID, FULL, PNAM(float priority, dflt 50.0), BNAM(→DLBR), QNAM(→QUST), DATA(4B), SNAM(U32 4-char code), TIFC(U32 info count) | DATA = TopicFlags(U8)+Category(U8)+Subtype(U16). SNAM stored as raw little-endian ASCII (e.g. `CUST`, `HELO`). |
