...
def convert_INFO(rec: dict, *, injected_ctdas: bytes = b'',
    ...
    subs += _info_tclt(rec, bark_dial_fids)
...
def _info_tclt(rec: dict, bark_dial_fids) -> bytes:
    """TCLT choice links (follow-up topics), in source order.

    A bark INFO keeps only choices pointing at a CONVERSATION topic; a choice
    into another bark is dropped (barks are split/merged by quest+subtype, so
    the link would dangle), as is a choice into a zero-INFO topic.

    See: docs/commentary/tes5_import_dialogue.md#info-tclt-choice-filter
    """
    def _keep_choice(cfid: int) -> bool:
        """True when this choice target survives the bark/empty filters."""
        if not cfid:
            return False
