...
def _quest_dnam(rec: dict) -> bytes:
    ...
    return struct.pack('<HBBII', flags, priority, 0, 0, 0)
...
def convert_QUST(rec: dict, fid_to_edid: dict = None,
    ...
    Order: EDID [VMAD] FULL DNAM NEXT [stages] [objectives] ANAM.
    """
    subs = b''
...
def convert_INFO(rec: dict, *, injected_ctdas: bytes = b'',
    ...
    injected_ctdas are the Skyrim-required gates (voice type / identity / stage /
    quest-running), already packed and placed BEFORE the translated TES4
    conditions so their OR chains stay isolated.
    """
    subs = b''
    edid = get_str(rec, 'EditorID')
    if edid:
...
def build_addtopic_stage_gates(by_type: dict) -> dict:
    """AddTopic visibility -> GetStage gates.

    Oblivion reveals a topic with a script `AddTopic T`. The controlling stage
    is found by pairing `AddTopic`/`SetStage` in the same INFO result script, or
    by the quest stage whose result script contains the `AddTopic`. Returns
    {topic_edid_lower: [(quest_edid_lower, min_stage), ...]} resolved to FormIDs
    below.

    This is the script-dependent AddTopic->conditions transform the skill flags:
    where we can read the script we recover the timing; where we can't, the
    topic still works via its quest-running ownership but loses exact timing.
    """
    re_setstage = re.compile(r'\bsetstage\s+(\w+)\s+(\d+)', re.IGNORECASE)
    re_addtopic = re.compile(r'\baddtopic\s+(\w+)', re.IGNORECASE)

    dial_edid_to_fid = {get_str(r, 'EditorID', '').lower(): get_formid(r, 'FormID')
                        for r in by_type.get('DIAL', [])
                        if get_str(r, 'EditorID')}
    quest_edid_to_fid = {get_str(r, 'EditorID', '').lower(): get_formid(r, 'FormID')
                         for r in by_type.get('QUST', [])
                         if get_str(r, 'EditorID')}

    raw = defaultdict(list)   # topic_edid -> [(quest_edid, stage)]

    # INFO result scripts: AddTopic + SetStage in the same script.
    for rec in by_type.get('INFO', []):
        ...
        if addtopics and setstages:
            ...
            for at in addtopics:
                raw[at.lower()].append((quest_edid.lower(), int(stage_str)))

    # Quest stage result scripts: AddTopic fires at that stage.
    for rec in by_type.get('QUST', []):
        ...
        for i in range(stage_count):
            stage_idx = get_int(rec, f'Stage[{i}].Index')
            scripts = [get_str(rec, f'Stage[{i}].ResultScript')]
            log_count = get_int(rec, f'Stage[{i}].LogCount')
            scripts += [get_str(rec, f'Stage[{i}].Log[{j}].ResultScript')
                        for j in range(log_count)]
            for script in scripts:
                if not script:
                    continue
                for at in re_addtopic.findall(script):
                    raw[at.lower()].append((quest_edid.lower(), stage_idx))

    # Resolve to FormIDs, keeping the minimum (earliest) stage per quest.
    result = {}
    for topic_edid, gate_list in raw.items():
        ...
        for q, stages in by_quest.items():
            ...
            qfid = quest_edid_to_fid.get(q)
            if qfid and min_stage > 0:
                gates.append((qfid, min_stage))
        if gates:
...
def build_dialog_groups(by_type: dict, writer, npc_to_vtyp: dict,
    ...
    stage_gates = build_addtopic_stage_gates(by_type)
    ...
    print(f"  Dialogue: {len(dials)} topics, {len(infos)} infos, "
          f"{len(npc_to_vtyp)} NPC->VTYP, {len(stage_gates)} AddTopic-gated topics")
    ...
    print(f"    topics={stats['topics']} infos={stats['infos']} "
          ...
          f"id-gated={stats['id_gated']} stage-gated={stats['stage_gated']} "
          f"quest-gated={stats['quest_gated']}")
...
def _build_one_topic(dial_rec, info_by_dial, writer, remap, offset,
    ...
    if not is_bark and child_infos:
        # A branch is "Normal" (reachable only via a parent's choice link, not
        # shown in the menu) only when the topic is BOTH a TCLT target AND a
        # Type-1 Conversation topic. Oblivion Type-0 "Topic" records are
        # player-facing top-level even when they are also choice targets, so
        # they stay Top-Level — otherwise ~1800 selectable topics would vanish
        # from the dialogue menu.
        is_linked = (dial_fid in tclt_targets
                     and dtype == DIAL_TYPE_CONVERSATION)
    ...
    # AddTopic stage gate for this topic (remapped quest FormIDs).
    raw_gates = stage_gates.get(dial_fid, [])
    stage_gate_bytes = b''
    if raw_gates:
        for idx, (qfid, min_stage) in enumerate(sorted(raw_gates)):
            is_last = (idx == len(raw_gates) - 1)
            stage_gate_bytes += pack_subrecord('CTDA', build_ctda(
                FUNC_GET_STAGE, param1=remap(qfid), comp_value=float(min_stage),
                operator=0x60, is_or=not is_last))
    ...
    for info_rec in child_infos:
        try:
            ...
            injected = _build_injected_ctdas(
                ...
                quest_gate_bytes, stage_gate_bytes, offset, stats)
...
def _build_injected_ctdas(info_rec, is_bark, npc_to_vtyp, topic_vtyps,
                          topic_npc_fids, quest_gate_bytes, stage_gate_bytes,
                          offset, stats):
    """Build the Skyrim-required gates for one INFO, ordered for OR-chain safety.

    Order (outermost AND first): [quest-running gate] [stage gate]
    ...
    return quest_gate_bytes + stage_gate_bytes + voice_bytes + id_bytes
