...
class _FakeWriter:
    """Minimal writer stub for dialogue group tests."""
    def __init__(self):
...
def _walk_records(blob):
    """Yield (sig, formid, full_record_bytes) for records in raw group
    content, descending into nested GRUPs."""
    pos = 0
...
class TestDialogueConversion:
    """Regression tests for dialogue conversion, pinned to byte layouts
    verified against real Skyrim.esm data and xEdit wbDefinitionsTES5."""

    def test_dial_data_byte_order(self):
        ...
        EXCEPTION_ACCESS_VIOLATION while initializing topics (the crash log
        shows the TESTopic* and its owning TESQuest*).
        """
    ...
    def test_classify_topic_categories_in_engine_range(self):
        """Every classification must produce a category the engine can index
        (vanilla uses 0-7) and a 4-byte SNAM."""
        from tes5_import.dialog_converter import classify_topic, _EDID_SUBTYPE
    ...
    def test_ctda_layout_matches_vanilla(self):
        """TES5 CTDA: 32 bytes; runOn=0/ref=0/param3=-1 tail like vanilla."""
        from tes5_import.dialog_conditions import build_ctda
        ctda = build_ctda(426, param1=0x01001234)
        assert len(ctda) == 32
        assert struct.unpack_from('<H', ctda, 8)[0] == 426
        assert struct.unpack_from('<I', ctda, 12)[0] == 0x01001234
        run_on, ref, p3 = struct.unpack_from('<IIi', ctda, 20)
        assert (run_on, ref, p3) == (0, 0, -1)

    def test_ctda_run_on_target_translated(self):
        """TES4 type bit 0x02 (Run on target) must become TES5 RunOn=1 with
        the bit cleared — in TES5 that bit means 'use aliases'."""
        from tes5_import.dialog_conditions import convert_ctda
        tes4 = struct.pack('<B3xIHHII4x', 0x02, 0x3F800000, 72, 0, 0x1234, 0)
        out = convert_ctda(tes4, offset=0)
        assert out is not None
        assert out[0] & 0x02 == 0
        assert struct.unpack_from('<I', out, 20)[0] == 1

    def test_ctda_use_global_flag_remaps_compvalue(self):
        """Only type bit 0x04 (Use Global) makes CompValue a FormID."""
        from tes5_import.dialog_conditions import convert_ctda
        glob = struct.pack('<B3xIHHII4x', 0x04, 0x00001234, 58, 0, 0x5678, 0)
        out = convert_ctda(glob, offset=1)
        assert struct.unpack_from('<I', out, 4)[0] == 0x01001234
        # NotEqual operator (0x20) is NOT Use Global — CompValue stays a float
        ne = struct.pack('<B3xIHHII4x', 0x20, 0x3F800000, 72, 0, 0x1234, 0)
        out = convert_ctda(ne, offset=1)
        assert struct.unpack_from('<I', out, 4)[0] == 0x3F800000

    def test_ctda_or_chain_repaired_after_drop(self):
        """Dropping the last condition of an OR chain must clear the dangling
        OR flag on the new last condition."""
        from tes5_import.dialog_conditions import convert_ctda_list
        keep_or = struct.pack('<B3xIHHII4x', 0x01, 0x3F800000, 72, 0, 0x1234, 0)
        dropped = struct.pack('<B3xIHHII4x', 0x00, 0x3F800000, 76, 0, 0, 0)
        rec = {'Condition[0].Raw': keep_or.hex(), 'Condition[1].Raw': dropped.hex()}
        out = convert_ctda_list(rec, offset=0)
        assert len(out) == 1
        assert out[0][0] & 0x01 == 0
    ...
    def test_quest_ownership_and_per_info_gating(self):
        ...
        try:
            ...
            assert 56 not in ctda_funcs(recs[('INFO', 0x010C0001)])
