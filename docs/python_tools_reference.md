...
- **Collision winding detector**: `python tools/collision_winding.py <nif_or_dir> [--converted] [--top N] [--workers N]` — finds inverted floor halves (up-facing + down-facing near-horizontal triangles sharing an edge), the "fall through half the floor" bug in Nehrim source meshes. `--converted` scans bhkCompressedMeshShape output instead of TES4-format source. Control test: a vanilla Oblivion tree should report only a handful of hits. See docs/nif_conversion_notes.md.
...
- **Navmesh reachability**: `python tools/navmesh_reach.py <esm> --from-ref <fid> --to-ref <fid> [--cell <fid> --components]` — decodes all NAVMs, builds the (mesh, component) graph over NVNM edge links + door-XTEL joins, and answers "can an actor path from A to B?" with per-component z-ranges/doors. Use FIRST when a quest NPC won't travel: it splits package bugs from navmesh bugs.
- **CK setting defaults**: `python tools/ck_settings_dump.py [--filter NavMeshGeneration]` — recovers CreationKit.exe's COMPILED-IN INI defaults by decoding each setting's dynamic-initializer thunk. Works for any `Name:Section` setting, not just navmesh ones.
- **CK string xrefs**: `python tools/ck_strref.py --pattern '<regex>'` — indexes every rip-relative reference from `.text` to matching strings in one pass. Progress/log strings are emitted in execution order, so this recovers pass ORDERING from a stripped binary.
- **CheckNavMesh on generated cells**: `python tools/navmesh_cell_check.py <EditorID> [...]` — runs the CK CheckNavMesh rules against `build.build_navmesh` output for named interior cells, WITHOUT needing a converted ESM. Seconds per cell, so generator fixes can be iterated directly; use `navmesh_check.py` on the ESM for the final gate.
- **CheckNavMesh (CK rule port)**: `python tools/navmesh_check.py <esm> [--verbose] [--rule NAME] [--csv out.csv] [--no-doors] [--all-portals]` — the Creation Kit's own navmesh validation rules (recovered from CreationKit.exe, see docs/ck_navmesh_generation.md), run against WRITTEN NVNM records: bad vertex/triangle/edge-link indices, degenerate tris, duplicate edge targets, asymmetric edge connections, downfacing + opposite-linked normals, portals to missing meshes/triangles/other worldspaces, NAVI registration, teleport doors with no Door Triangle, and the CK's 3500/5000 triangle-count warnings. This is the CORRECTNESS gate (`navmesh_audit.py` is the quality metric). Exit code 1 on any non-`_WARN` defect. Baseline: Skyrim.esm 225 findings / 2.67M tris, Dawnguard.esm 46 — a run that reports far more than that on vanilla means the TOOL is wrong, not the data.
